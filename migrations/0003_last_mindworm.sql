...
CREATE TABLE `__new_fill_ups` (
	...
	CONSTRAINT "fill_ups_trip_type_check" CHECK("__new_fill_ups"."trip_type" is null or "__new_fill_ups"."trip_type" in ('road','city'))
);
--> statement-breakpoint
INSERT INTO `__new_fill_ups`("id", "vehicle_id", "created_at", "date", "odometer_km", "liters", "total_price", "fuel_type", "is_full_tank", "trip_type", "notes") SELECT "id", "vehicle_id", "created_at", "date", "odometer_km", "liters", "total_price", "fuel_type", "is_full_tank", NULL, "notes" FROM `fill_ups`;--> statement-breakpoint
